...
def frage_mit_default(text, default, validierer=None, leer_erlaubt=False):
    """Enter = aktuellen Wert behalten. Bei leer_erlaubt=True leert '-' den Wert."""
    while True:
...
def waehle_aus_liste(eintraege, aktion_text, beschriftung=str):
    ...
    auswahl = frage(
        f"\nWelchen Eintrag {aktion_text}? (1-{len(eintraege)}): ",
...
def menue(titel, punkte, mit_zurueck=True):
    ...
    zurueck oder None bei 0/Zurueck."""
    print(f"\n{titel}")
    ...
    wahl = frage("Auswahl: ", lambda a: None if a in gueltig else "Ungueltige Auswahl.")
    if wahl == "0":
        return None
...
def rueckgaengig():
    ...
    if not frage_ja("Wirklich rueckgaengig machen? (j/n): "):
        print("Abgebrochen.")
        return
